...
def rodar_script(connection):
    os.system('cls')
    try:
        opcao = input("Selecione uma opção:\n1 - Enviar conteúdo de um arquivo TXT\n2 - Digitar os comandos manualmente\nOpção: ")

        if opcao == "1":
            diretorio_local = input("> Caminho local do arquivo de script: ")

            # Verifica se o arquivo existe
            if not os.path.isfile(diretorio_local):
                return {"error": f"Erro: Arquivo '{diretorio_local}' não encontrado."}
            ...
            return {
                "resultado": {
                    "Mensagem": f"Script '{diretorio_local}' enviado para o equipamento.",
            ...
            return {
                "Resultado": {
                    "Mensagem": "Comandos digitados enviados para o equipamento.",
                    "Resposta": response_formatted
                }
            }
    ...
    def post(self):
        ...
        if opcao == "menu":
            ...
            if connection:
                ...
                return {'error': 'Equipamento ou Opção não reconhecidos'}, 400
